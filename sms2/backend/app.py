...
def login_required(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not uid: return jsonify({'success': False, 'message': 'Chưa đăng nhập'}), 401
...
def role_required(*roles):
    def dec(f):
        ...
        def wrapper(*args, **kwargs):
            uid, role = decode_token()
            if not uid: return jsonify({'success': False, 'message': 'Chưa đăng nhập'}), 401
            if role not in roles: return jsonify({'success': False, 'message': 'Không có quyền'}), 403
            request.uid = uid; request.role = role
            return f(*args, **kwargs)
        return wrapper
    return dec
...
def register():
    ...
    try:
        with conn.cursor() as c:
            c.execute("SELECT id FROM users WHERE email=%s", (email,))
            if c.fetchone(): return jsonify({'success': False, 'message': 'Email đã được sử dụng'}), 400
            c.execute("INSERT INTO users(full_name,email,password,role) VALUES(%s,%s,%s,%s)",
                      (name, email, hash_pw(pw), role))
            conn.commit()
...
def update_profile():
    ...
    pw      = d.get('password') or ''
    if not name: return jsonify({'success': False, 'message': 'Họ tên không được để trống'}), 400
    conn = get_db()
    try:
        with conn.cursor() as c:
            ...
            if pw:
                if len(pw) < 6: return jsonify({'success': False, 'message': 'Mật khẩu phải >= 6 ký tự'}), 400
...
def create_course():
    ...
    if not code or not name: return jsonify({'success': False, 'message': 'Thiếu mã môn hoặc tên môn'}), 400
    conn = get_db()
    try:
        with conn.cursor() as c:
            c.execute("SELECT id FROM courses WHERE code=%s", (code,))
            if c.fetchone(): return jsonify({'success': False, 'message': 'Mã môn đã tồn tại'}), 400
            c.execute("INSERT INTO courses(code,name,description,credits,max_students,teacher_id,semester) VALUES(%s,%s,%s,%s,%s,%s,%s)",
...
def update_course(cid):
    d = request.get_json(force=True) or {}
    name = (d.get('name') or '').strip()
    if not name: return jsonify({'success': False, 'message': 'Tên môn không được để trống'}), 400
    conn = get_db()
    try:
        with conn.cursor() as c:
...
def enroll():
    ...
    if not cid: return jsonify({'success': False, 'message': 'Thiếu course_id'}), 400
    conn = get_db()
    try:
        with conn.cursor() as c:
            c.execute("SELECT id,max_students FROM courses WHERE id=%s", (cid,))
            course = c.fetchone()
            if not course: return jsonify({'success': False, 'message': 'Môn học không tồn tại'}), 404
            ...
            if c.fetchone()['cnt'] >= course['max_students']:
                return jsonify({'success': False, 'message': 'Lớp học đã đầy'}), 400
            c.execute("SELECT id,status FROM enrollments WHERE student_id=%s AND course_id=%s", (sid, cid))
            ex = c.fetchone()
            if ex:
                if ex['status'] == 'active': return jsonify({'success': False, 'message': 'Đã đăng ký môn học này rồi'}), 400
...
def drop_enrollment(eid):
    ...
    try:
        with conn.cursor() as c:
            ...
            if not c.fetchone(): return jsonify({'success': False, 'message': 'Không tìm thấy'}), 404
...
def get_course_students(cid):
    if request.role == 'teacher':
        ...
        if not course or course['teacher_id'] != request.uid:
            return jsonify({'success': False, 'message': 'Không có quyền xem lớp này'}), 403
    rows = qall('''
...
def save_grade():
    ...
    try: gval = float(gval)
    except: return jsonify({'success': False, 'message': 'Điểm không hợp lệ'}), 400
    if not 0 <= gval <= 10: return jsonify({'success': False, 'message': 'Điểm phải từ 0 đến 10'}), 400
    if request.role == 'teacher':
        c = qone("SELECT teacher_id FROM courses WHERE id=%s", (cid,))
        if not c or c['teacher_id'] != request.uid:
            return jsonify({'success': False, 'message': 'Không có quyền nhập điểm môn này'}), 403
    conn = get_db()
    try:
        with conn.cursor() as c:
...
def create_user():
    ...
    if not name or not email: return jsonify({'success': False, 'message': 'Thiếu thông tin bắt buộc'}), 400
    if role not in ('student','teacher','admin'): return jsonify({'success': False, 'message': 'Vai trò không hợp lệ'}), 400
    conn = get_db()
    try:
        with conn.cursor() as c:
            c.execute("SELECT id FROM users WHERE email=%s", (email,))
            if c.fetchone(): return jsonify({'success': False, 'message': 'Email đã tồn tại'}), 400
...
def update_user(tid):
    ...
    name  = (d.get('full_name') or '').strip()
    email = (d.get('email') or '').strip().lower()
    if not name or not email: return jsonify({'success': False, 'message': 'Thiếu thông tin'}), 400
    conn = get_db()
    try:
        with conn.cursor() as c:
            c.execute("SELECT id FROM users WHERE email=%s AND id!=%s", (email, tid))
            if c.fetchone(): return jsonify({'success': False, 'message': 'Email đã được dùng bởi tài khoản khác'}), 400
...
@role_required('admin')
def delete_user(tid):
    if tid == request.uid: return jsonify({'success': False, 'message': 'Không thể xóa chính mình'}), 400
